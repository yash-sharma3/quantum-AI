...
class AIKeysManager:
    def __init__(self):
        ...
        self.your_number = "whatsapp:+91 Your Number"
        self.twilio_whatsapp_number = "whatsapp:Twillio Number"
        ...
        self.father_email = "Your Fathers Mail"
        self.mother_email = "Your Mothers Mail"
    ...
    def get_twilio_whatsapp_number(self):
        return self.twilio_whatsapp_number
    
    def get_your_number(self):
        return self.your_number
    ...
    def get_father_email(self):
        return self.father_email

    def get_mother_email(self):
...
def capture_intruder():
    ...
    if ret:
        ...
        folder = r"D:\YashSharma\Python\Jarvis\intruder_photos"
        os.makedirs(folder, exist_ok=True)
        file_path = os.path.join(folder, f"intruder_{timestamp}.jpg")
        cv2.imwrite(file_path, frame)
        print(f"🚨 Intruder photo saved: {file_path}")
        return file_path
    else:
        print("❌ Failed to capture photo")
        return None

def send_intruder_email(photo_path):
    from_addr = keys.get_email_user()
    to_addr = keys.get_email_user()
    subject = "🚨 Intruder Alert – Wrong Password Entered!"
    body = "Jarvis detected a wrong password attempt. See attached photo."
    ...
    with open(photo_path, "rb") as attachment:
        ...
        mime_base.add_header("Content-Disposition", f"attachment; filename={os.path.basename(photo_path)}")
...
# Password check
with open(r"D:\YashSharma\Python\Jarvis\Encrypted_Vault\password.txt", "r") as file:
...
def get_weather(city):
    API_KEY = keys.get_openweather_api_key()
...
# Define the common path for UI elements
UI_ELEMENTS_PATH = "D:/YashSharma/Python/Jarvis/UI_Elements/"

# Define the path for the mic state file
MIC_DATA_PATH = "D:/YashSharma/Python/Jarvis/mic.data"
PRINT_JAR_DATA_PATH = "D:/YashSharma/Python/Jarvis/print_jar.data"


class AnimatedGIF(tk.Label):
    """
    A Tkinter Label subclass that can display an animated GIF.
    """
    def __init__(self, master, path, size=None, delay=50, **kwargs):
        full_path = os.path.join(UI_ELEMENTS_PATH, path)
        if not os.path.exists(full_path):
            super().__init__(master, text=f"{path} Not Found", fg="red", bg="black")
            self.frames = []
            return
        ...
        try:
            for frame in ImageSequence.Iterator(im):
                ...
                if size:
                    if "Jarvis.gif" in path:
                        side = min(size)
                        fr = fr.resize((side, side), Image.Resampling.LANCZOS)
                    else:
                        fr = fr.resize(size, Image.Resampling.LANCZOS)
                self.frames.append(ImageTk.PhotoImage(fr))
        except Exception as e:
            super().__init__(master, text="GIF Load Error", fg="red", bg="black")
            self.frames = []
            return
        
        self.delay = delay
        self.idx = 0
        super().__init__(master, image=self.frames[0], bg="black", **kwargs)
        self.play()
...
class JarvisGUI:
    """
    The main class for the J.A.R.V.I.S. GUI.
    """
    def __init__(self, root):
        ...
        # Load mic button images
        self.mic_off_image = self.load_image("Mic_off.png", (100, 100))
        self.mic_on_image = self.load_image("Mic_on.png", (100, 100))
    ...
    def create_widgets(self):
        ...
        self.bg_image = Image.open(os.path.join(UI_ELEMENTS_PATH, "fram1.png"))
        self.bg_photo = ImageTk.PhotoImage(self.bg_image)
        self.bg_label = tk.Label(self.root, image=self.bg_photo, bg="black")
        self.bg_label.place(x=0, y=0, relwidth=1, relheight=1)

        # Central J.A.R.V.I.S. GIF on top of the background
        self.jarvis_gif = AnimatedGIF(self.bg_label, "Jarvis.gif", size=(700, 700), delay=80)
        self.jarvis_gif.place(relx=0.5, rely=0.45, anchor="center")
        
        # Iron Man GIF on top of the background
        self.ironman_gif = AnimatedGIF(self.bg_label, "ironman1.gif", size=(300, 350), delay=70)
        self.ironman_gif.place(relx=0.80, rely=0.65, anchor="center")

        self.ironman_circle_gif = AnimatedGIF(self.bg_label, "iron-man.gif", size=(200, 250), delay=70)
        self.ironman_circle_gif.place(relx=0.85, rely=0.20, anchor="center")

        self.jar_circle_gif = AnimatedGIF(self.bg_label, "ringjar.gif", size=(400, 400), delay=70)
        self.jar_circle_gif.place(relx=0.3, rely=0.35, anchor="ne")


        # Mic Button on top of the background
        self.mic_button = tk.Button(self.bg_label, image=self.mic_off_image, bd=0, bg="black", activebackground="black")
        self.mic_button.place(relx=0.5, rely=0.85, anchor="center")

        # New terminal-like text display box
        # This frame holds the Text widget and Scrollbar with a golden border
        self.text_frame = tk.Frame(self.bg_label, bd=2, relief="solid", highlightbackground="#FFD700", highlightcolor="#FFD700", highlightthickness=2)
        self.text_frame.place(x=70, y=self.root.winfo_screenheight() - 40, anchor="sw", width=500, height=200)
        ...
        self.jarvis_text_display = tk.Text(self.text_frame, yscrollcommand=self.scrollbar.set, fg="white", bg="black", font=("Arial", 14), wrap="word", relief="flat")
        self.jarvis_text_display.pack(side="left", fill="both", expand=True)
        # Crucial step: Configure the scrollbar to control the text widget's yview
        self.scrollbar.config(command=self.jarvis_text_display.yview)

        # Insert initial text
        self.jarvis_text_display.insert("end", "Awaiting Command...")
        self.jarvis_text_display.config(state="disabled") # Make it read-only
        self.create_buttons()
        self.create_labels()
    ...
    def create_labels(self):
        # Time Label
        self.time_label = tk.Label(self.root, text="", fg="#FFD700", bg="black", font=("Arial", 14), justify="left")
        self.time_label.place(x=130, y=230)

        # Weather Label
        self.weather_label = tk.Label(self.root, text="", fg="#FFD700", bg="black", font=("Arial", 14), justify="left")
        self.weather_label.place(x=130, y=260)

        # Power Label (for battery)
        self.power_label = tk.Label(self.root, text="", fg="#FFD700", bg="black", font=("Arial", 14), justify="left")
        self.power_label.place(x=130, y=290)

        # Call the update function to populate the labels
    ...
    def update_info_labels(self):
        ...
        if battery:
            ...
            power_status = f"Battery: {battery.percent}% ({plugged_in})"
        else:
            power_status = "Battery: N/A"
        self.power_label.config(text=power_status)

        # Schedule the next update
        self.root.after(30000, self.update_info_labels) # Update every 30 seconds
    ...
    def create_buttons(self):
        ...
        button_frame.place(relx=0.2, rely=0.15, anchor="ne")

        # Open Workspace button
        open_button = tk.Button(button_frame, text="Open Workspace", command=self.open_my_workspace, bg="#FFD700", fg="black", font=("Arial", 12))
        open_button.pack(side="left", padx=10)

        # Exit button
        exit_button = tk.Button(button_frame, text="Exit", command=self.exit_app, bg="#FFD700", fg="black", font=("Arial", 12))
    ...
    def open_my_workspace():
        # 1. Open jar1.py
        sp.Popen(["D:\YashSharma\Python\Jarvis\jar test.py", "D:\YashSharma\Python\Jarvis\jar1.py"], shell=True)

        # Small delay so Python script launches before browser opens
        time.sleep(1)

        # 2 & 3. Open in Chrome
        # Replace with your Chrome executable path
        chrome_path = r"C:\Program Files\Google\Chrome\Application\chrome.exe %s"
        chrome = webbrowser.get(chrome_path)

        urls = [
            "https://mail.google.com",     # Email
            "https://gemini.google.com",    # First URL
            "https://chatgpt.com"     # Second URL
        ]

        # Open first URL in a new Chrome window
        chrome.open(urls[0], new=1)

        # Open other URLs in new tabs of same Chrome window
        for url in urls[1:]:
    ...
    def resize_background(self, event):
        ...
        self.text_frame.place(x=20, y=new_height - 20, anchor="sw")

    def get_mic_state_from_file(self):
        """Reads the mic state from a file."""
        try:
            with open(MIC_DATA_PATH, 'r') as f:
                state = f.read().strip()
    ...
    def get_text_from_file(self):
        """Reads text from the print_jar.data file."""
        try:
            with open(PRINT_JAR_DATA_PATH, 'r', encoding='latin-1') as f:
                return f.read().strip()
        except FileNotFoundError:
            return "File Not Found: D:/YashSharma/Python/Jarvis/print_jar.data"
        except Exception as e:
            return f"Error reading file: {e}"

    def update_ui_elements(self):
        """Periodically checks and updates all UI elements."""
        if self.get_mic_state_from_file():
        ...
        current_scroll_pos = self.jarvis_text_display.yview()
        
        text_content = self.get_text_from_file()
        self.jarvis_text_display.config(state="normal")
        self.jarvis_text_display.delete(1.0, "end")
        self.jarvis_text_display.insert("end", text_content)
        self.jarvis_text_display.config(state="disabled")
        
        # Restore the scroll position after updating the content
        self.jarvis_text_display.yview_moveto(current_scroll_pos[0])
...
ui_thread = threading.Thread(target=run_gui_thread, daemon=True)
ui_thread.start()



def write_mic_state(state):
    """Writes the mic state to the mic.data file."""
    try:
        with open(MIC_DATA_PATH, 'w') as f:
            f.write(state)
    except Exception as e:
        print(f"Error writing to mic.data: {e}")

def write_Jarvis_response(state):
    """Writes the Jarvis response state to the print_jar.data file."""
    try:
        # Open the file with 'utf-8' encoding to support all characters
        with open(PRINT_JAR_DATA_PATH, 'w', encoding='utf-8') as f:
            f.write(state)
    except Exception as e:
        # It's also good practice to check if you're writing to the correct file
        print(f"Error writing to print_jar.data: {e}")
...
VAULT_FILE = "D:/YashSharma/Python/Jarvis/Encrypted_Vault/notes_vault.enc"
KEY_FILE = "D:/YashSharma/Python/Jarvis/Encrypted_Vault/vault_key.key"

# Generate a key (do only once and store safely)
def generate_key():
...
# Example usage:
# generate_key()  # Run once to create vault key
# encrypt_note("My ATM PIN is 1234")
# decrypt_notes()
...
db = MySQLdb.connect("localhost", keys.get_database_user(), keys.get_database_pass(), keys.get_database_name())
cursor = db.cursor()
...
def sendmessage(message):
    ...
    from_=keys.get_twilio_whatsapp_number(),
    body=f'{message}',
    to=keys.get_your_number()
    )
...
def find_answer_local(question):
    """Search qna.txt for the answer to a given question (supports multi-line answers)."""
    question = question.lower().strip()
    try:
        with open("D:/YashSharma/Python/Jarvis/Local_Storage/qna.txt", "r", encoding="utf-8") as f:
            data = f.read().split("---")
    except FileNotFoundError:
        return None

    for qa in data:
        if "question:" in qa.lower():
            lines = qa.strip().split("\n")
            q_line = lines[0].replace("Question:", "").strip().lower()
            if q_line == question:
                # Join everything after "Answer:" as the answer
                answer_lines = []
                found = False
                for line in lines[1:]:
                    if line.strip().lower().startswith("answer:"):
                        ...
                        found = True
                    elif found:  # capture remaining lines as part of answer
                        answer_lines.append(line.strip())
                return "\n".join(answer_lines).strip()
    return None


def save_to_qna(question, answer):
    """Save new Q&A to qna.txt (multi-line safe)."""
    with open("D:/YashSharma/Python/Jarvis/Local_Storage/qna.txt", "a", encoding="utf-8") as f:
...
KEY_FILE = "D:/YashSharma/Python/Jarvis/Encrypted_Vault/vault_key.key"
STATUS_FILE = "D:/YashSharma/Python/Jarvis/Encrypted_Vault/protocol_status.enc"
...
def encrypt_status(status):
    """Encrypt and store protocol status."""
    key = load_key()
    fernet = Fernet(key)
    encrypted_data = fernet.encrypt(status.encode())
    with open(STATUS_FILE, "wb") as status_file:
        status_file.write(encrypted_data)

def decrypt_status():
    """Decrypt and read protocol status."""
    if not os.path.exists(STATUS_FILE):
        return "Enabled"  # Default
    key = load_key()
    fernet = Fernet(key)
    with open(STATUS_FILE, "rb") as status_file:
        encrypted_data = status_file.read()
    return fernet.decrypt(encrypted_data).decode()




with open('D:\YashSharma\Python\Jarvis\Encrypted_Vault\password.txt', 'r') as file:
    # Read the password
    paw= file.read().strip()
...
def disable_all_protocols():
    password = input("Enter password to disable protocols: ")

    if password == paw:  # Change this!
        encrypt_status("Disabled")
        print("✅ Protocols disabled.")
        speak("Protocols have been disabled")
    else:
        print("❌ Wrong password. Protocols remain active.")

def enable_all_protocols():
    pin = input("Enter PIN to enable protocols: ")
    if pin == paw:  # Change this!
        encrypt_status("Enabled")
        print("✅ Protocols enabled.")
        speak("Protocols have been enabled")
    else:
        print("❌ Wrong PIN. Protocols remain disabled.")
...
#########################################################################################################################################################
        ...
        def summarize_with_gemini():
            ...
            :param model: The Gemini model object (already initialized in your jar1.py).
            :return: Summary string.
            """


            prompt = input("Enter the text to summarize: ")
            summarizer =f"{prompt} - summarize this in 5-10 lines or sentences"
            try:
                response = chat_with_gemini(summarizer)
                print(response)
                speak("Sir this is the summarized content for you")
            except Exception as e:
...
#########################################################################################################################################################
        ...
        def summarize_website(url):
            try:
                ...
                text = re.sub(r'\n+', '\n', text).strip()  # Clean multiple newlines



                # Read only first part if text is huge
                preview = "\n".join(text.split("\n")[:50])
                summary = chat_with_gemini(f"generate a 10 line summary for {preview}")
                speak("sir here is an website content for you")
                print("📄 Website content:\n", preview)

                speak("Now getting summary for you sir")
...
#########################################################################################################################################################
        ...
        def InternetSpeed():
            write_Jarvis_response("Checking internet speed...")
            ...
            try:
                ...
                write_Jarvis_response(f"Download Speed: {dl:.2f} Mbps | Upload Speed: {up:.2f} Mbps")
        ...
        def temperature():
            try:
                ...
                if temp_element:
                    ...
                    write_Jarvis_response(f"Current temperature in {city} is {temp}")
                else:
...
#########################################################################################################################################################
        ...
        def ty_speed_test():
            ...
            def typing_speed_test():
                ...
                write_Jarvis_response(f"Your typing speed is: {words_per_minute:.2f} words per minute")
...
#########################################################################################################################################################
        ...
        # Morse code dictionary
            ...
            # Example usage
            if __name__ == "__main__":
                write_Jarvis_response("Enter text to convert to Morse code")
                ...
                write_Jarvis_response("Morse Code:", morse_code)
        ...
        def decode_morse_code():
            ...
            # Example usage
            if __name__ == "__main__":
                write_Jarvis_response("Enter Morse code to convert to text")
                ...
                print("Text:", text)
                write_Jarvis_response("Text:", text)
...
#########################################################################################################################################################
        ...
        def create_qr_code():
                write_Jarvis_response("sir please enter url")
                ...
                write_Jarvis_response("Sir by which name you have to save it , sir")
                ...
                write_Jarvis_response("Sir, the QR code has been successfully saved.")
...
#########################################################################################################################################################
        ...
        def open_my_workspace():
            # 1. Open jar1.py
            write_Jarvis_response("Opening Workspace")
            sp.Popen(["D:\YashSharma\Python\Jarvis\jar test.py", "D:\YashSharma\Python\Jarvis\jar1.py"], shell=True)
...
#########################################################################################################################################################
        ...
        # Base folder for storing Research Mode files
        BASE_DIR = r"D:/YashSharma/Python/Jarvis/AI_Research_Mode"
        os.makedirs(BASE_DIR, exist_ok=True)

        def fetch_wikipedia_data(topic):
            """Fetch summary + full content from Wikipedia."""
            try:
        ...
        def fetch_internet_data(topic, limit=3):
            """Fetch top articles from the web (basic scraping)."""
            query = topic.replace(" ", "+")
            url = f"https://www.google.com/search?q={query}"
            headers = {"User-Agent": "Mozilla/5.0"}
            html = requests.get(url, headers=headers).text
            soup = BeautifulSoup(html, "lxml")  # ✅ fixed

            links = []
            for g in soup.find_all("a", href=True):
                href = g['href']
                if "http" in href and "google" not in href and len(links) < limit:
                    try:
                        page = requests.get(href, headers=headers, timeout=5).text
                        text = BeautifulSoup(page, "lxml").get_text()  # ✅ fixed
                        links.append(f"URL: {href}\n{text[:5000]}...\n")  # limit text
                    except:
                        pass
            return "\n\n".join(links)

        def research_mode(topic, gemini_summarizer):
            """Full AI Research Mode pipeline."""
            print(f"[Jarvis] Collecting raw data on: {topic} ...")

            # 1. Collect Raw Data
            wiki_data = fetch_wikipedia_data(topic)
            web_data = fetch_internet_data(topic, limit=5)

            raw_data = f"=== WIKIPEDIA DATA ===\n{wiki_data}\n\n=== INTERNET DATA ===\n{web_data}"
            
            raw_filename = os.path.join(BASE_DIR, f"Raw_{topic.replace(' ', '_')}.txt")
            with open(raw_filename, "w", encoding="utf-8") as f:
                f.write(raw_data)

            print(f"[Jarvis] Raw data saved to {raw_filename}")

            # 2. Summarize using Gemini 2.0 Flash
            summary_prompt = f"""
            You are an expert researcher. I will give you raw text collected from
            Wikipedia + the Internet on the topic: {topic}.
            Please give me a VERY DETAILED and CORRECT summary, covering:
            - Introduction
            - Key Concepts
            ...
            {raw_data[:15000]}  # (truncate if too large for the model)
            """

            final_summary = gemini_summarizer(summary_prompt)

            final_filename = os.path.join(BASE_DIR, f"Final_{topic.replace(' ', '_')}.txt")
            with open(final_filename, "w", encoding="utf-8") as f:
                f.write(final_summary)

            print(f"[Jarvis] Final summary saved to {final_filename}")
...
#########################################################################################################################################################
        ...
        def screen_intelligence_image():
            try:
                # 1. Capture screenshot
                
                write_Jarvis_response("Sir, clicking image...")
                print("📸 Sir, clicking image...")
                screenshot_path = "D:\YashSharma\Python\Jarvis\Screenshots\screen_summary.jpg"
                pyautogui.screenshot(screenshot_path)

                # 2. Configure Gemini API
                write_Jarvis_response("Sir, uploading image for analysis...")
                print("☁️ Sir, uploading image for analysis...")
                genai.configure(api_key=keys.get_gemini_api_key())

                # 3. Load multimodal model
                model = genai.GenerativeModel("gemini-1.5-flash")

                # 4. Open screenshot and send with instruction
                with open(screenshot_path, "rb") as img_file:
                    response = model.generate_content([
                        "You are Jarvis AI. Analyze this screenshot and tell me what’s on the screen. "
                        "If it contains a question or problem, answer it directly.",
                        {"mime_type": "image/png", "data": img_file.read()}
                    ])

                write_Jarvis_response("Here is the summary for you, Sir:")
                print("📑 Here is the summary for you, Sir:\n")
                speak("Here is the summary for you sir")
                # 5. Show + Speak result

                print("\n🤖 Analysis:\n", response.text)
                write_Jarvis_response(response.text)
                speak("sir should i read it")
                while True:
                    print("Please Answer in yes or no")
                    accept_status=takeCommand()

                    if "yes" in accept_status.lower():
                        speak(response.text)
                        break
                    elif "no" in accept_status.lower():
                        speak("Ok as you wish Sir")
                        break


            except Exception as e:
                print(f"❌ Screen Intelligence Error: {e}")
...
#########################################################################################################################################################
        ...
        def generate_barcode():
            ...
            write_Jarvis_response("sir what name you want it to be saved")
...
# Sleep Function
        ...
        if "jarvis sleep" in command.lower():
            write_Jarvis_response("ok Sir going to sleep")
            ...
            while True:
                ...
                if "wake up jarvis" in command.lower():
                   write_Jarvis_response("i am back sir")
                   speak("i am back sir")
                   break
...
#########################################################################################################################################################
            ...
            write_Jarvis_response("sir please enter image path")
...
#########################################################################################################################################################
        elif 'open my workspace'in command:
            status = decrypt_status()
            if status == "Disabled":
                print(f"⚠ Protocols are disabled")
            else:

                open_my_workspace()


        elif 'open workspace'in command:
            status = decrypt_status()
            if status == "Disabled":             
                print(f"⚠ Protocols are disabled")
            else:
        ...
        elif 'open my work space'in command:
            status = decrypt_status()
            if status == "Disabled":
                print(f"⚠ Protocols are disabled")
            else:
        ...
        elif 'open work space'in command:
            status = decrypt_status()
            if status == "Disabled":
                print(f"⚠ Protocols are disabled")
            else:
...
#########################################################################################################################################################
            ...
            write_Jarvis_response(f"sir our system have {persentage} percent battery")
...
#########################################################################################################################################################
        ...
        elif "summarise text" in command or "summarise paragraph" in command:
            summarize_with_gemini()

        elif "summarise content" in command or "summarise para" in command:
            summarize_with_gemini()

        elif "summarize text" in command or "summarize paragraph" in command or "summurized content" in command:
            summarize_with_gemini()

        elif "summarize text" in command or "summarize para" in command :
            summarize_with_gemini()

        elif "summaries text" in command or "summaries paragraph" in command or "summaries content" in command:
            summarize_with_gemini()

        elif "summaries text" in command or "summaries para" in command :
            summarize_with_gemini()
...
#########################################################################################################################################################
        ...
        elif "should i go" in command or "can i go out" in command:
            wheather_suggestion()
...
#########################################################################################################################################################
            ...
            write_Jarvis_response("Sir please enter the name of the module")
            speak("Sir please enter the name of the module")
            modu=input("Enter name of Module: ")
            os.system(f"pip install {modu}")
            write_Jarvis_response(f"done sir module {modu} is installed")
            speak(f"done sir module {modu} is installed")

        elif "install Python library" in command:
            write_Jarvis_response("Sir please enter the name of the library")
            speak("Sir please enter the name of the library")
            modu=input("Enter name of Module: ")
            os.system(f"pip install {modu}")
            write_Jarvis_response(f"done sir module {modu} is installed")
            speak(f"done sir module {modu} is installed")
...
#########################################################################################################################################################
            ...
            active_window = gw.getActiveWindow()
            if active_window !="":
                write_Jarvis_response(f"closing window: {active_window.title}")
                print(f"closing window: {active_window.title}")
                speak(f"closing window: {active_window.title}")
                pyautogui.hotkey("alt","f4")

            if active_window =="":
                speak("you are on the desktop")
                write_Jarvis_response("you are on the desktop")
...
#########################################################################################################################################################
        ...
        elif "encrypt note" in command or "incrypt note" in command:
            write_Jarvis_response("Sir what is the note")
            speak("Sir what is the note")
            while True:
                command=takeCommand()
                if command != None:
                    write_Jarvis_response(f"Sir finally encrypting {command}")
                    speak(f"Sir finally encrypting {command}")
                    encrypt_note(command)
                    break

        elif "encrypt notes" in command or "incrypt notes" in command:
            write_Jarvis_response("Sir what is the note")
            speak("Sir what is the note")
            while True:
                command=takeCommand()
        ...
        elif "decrypt note" in command:
            write_Jarvis_response("sir, Decrypting notes")
            speak("sir, Decrypting notes")
            decrypt_notes()

        elif "decrypt notes" in command:
            write_Jarvis_response("sir, Decrypting notes")
            speak("sir, Decrypting notes")
            decrypt_notes()
...
#########################################################################################################################################################
            ...
            write_Jarvis_response("Recycle Bin Recycled")
...
#########################################################################################################################################################
            ...
            write_Jarvis_response("You told me to remember that"+rememberMessage)
...
#########################################################################################################################################################
            ...
            if random.choice(list1)== "1":
                write_Jarvis_response("its Heads")
                print("its Heads")
                speak("its Heads")

            elif random.choice(list1)== "2":
                write_Jarvis_response("its Tails")
                print("its Tails")
                speak("its Tails")
...
#########################################################################################################################################################
            ...
            write_Jarvis_response("What would you like to search on Amazon")
...
#########################################################################################################################################################
        ...
        elif "deep search" in command.lower():
            write_Jarvis_response("What topic should I research?")
            speak("What topic should I research?")
            topic = input("Enter Topic To Reserch On:")  # or however you get voice input
            speak(f"Researching on {topic}")
            final_file = research_mode(topic, chat_with_gemini)
            write_Jarvis_response(f"Research completed. I have saved the final summary in {final_file}")
            speak(f"Research completed. I have saved the final summary in {final_file}")
            os.startfile(final_file)  # auto-open in Notepad
...
#########################################################################################################################################################
                ...
                body = takeCommand()
                write_Jarvis_response("I've sent the email sir.")
                speak("I've sent the email sir.")

        elif "send email to papa" in command:
                receiver_address = keys.get_father_email()
                speak("What should be the subject sir?")
                subject = takeCommand()
                speak("What is the message sir?")
                body = takeCommand()
                ...
                receiver_address = keys.get_mother_email()
                speak("What should be the subject sir?")
                subject = takeCommand()
                speak("What is the message sir?")
                body = takeCommand()
...
#########################################################################################################################################################
        ...
        elif 'activate protocol 2'in command:
            status = decrypt_status()
            if status == "Disabled":
                print(f"⚠ Protocols are disabled")
            else:
        ...
        elif 'activate protocol 3' in command:
            status = decrypt_status()
            if status == "Disabled":
                print(f"⚠ Protocols are disabled")
            else:
        ...
        elif 'detail protocol 7' in command:
            status = decrypt_status()
            if status == "Disabled":
                print(f"⚠ Protocols are disabled")
            else:
        ...
        elif 'activate protocol 6' in command:
            status = decrypt_status()
            if status == "Disabled":
                print(f"⚠ Protocols are disabled")
            else:
        ...
        elif "enable all protocols" in command:
            enable_all_protocols()

        elif"enable protocols" in command:
            enable_all_protocols()
            
        elif "enable all the protocols" in command:
            enable_all_protocols()

        elif"enabled protocols" in command or "enabled all the protocols" in command or "enabled all protocols" in command:
            enable_all_protocols()


        elif"disable protocols" in command or "disable all the protocols" in command or "disable all protocols" in command:
            disable_all_protocols()

        elif"disabled protocols" in command or "disabled all the protocols" in command or "disabled all protocols" in command:
            disable_all_protocols()
...
#########################################################################################################################################################
                    ...
                    write_Jarvis_response(f"Sir, the time is {strTime}")
...
#########################################################################################################################################################
                ...
                write_Jarvis_response(location)
...
#########################################################################################################################################################
                ...
                write_Jarvis_response("sir here you can register and then prepare the speech")
...
#########################################################################################################################################################
            ...
            os.system("cls")
            write_Jarvis_response("Screen Cleared Sir")
...
#########################################################################################################################################################
        ...
        elif 'tell me about screen' in command:
            screen_intelligence_image()

        elif 'tell about screen' in command:
            screen_intelligence_image()

        elif 'tell me screen' in command:
            screen_intelligence_image()

        elif 'detail the screen' in command:
            screen_intelligence_image()

        elif 'detail screen' in command:
            screen_intelligence_image()

        elif 'detail about screen' in command:
            screen_intelligence_image()

        elif 'detail about the screen' in command:
            screen_intelligence_image()

        elif 'screen detail' in command:
            screen_intelligence_image()

        elif 'details the screen' in command:
            screen_intelligence_image()

        elif 'details screen' in command:
            screen_intelligence_image()

        elif 'details about screen' in command:
            screen_intelligence_image()

        elif 'details about the screen' in command:
            screen_intelligence_image()

        elif 'screen details' in command:
            screen_intelligence_image()

        elif 'details on the screen' in command:
            screen_intelligence_image()
...
#########################################################################################################################################################
            ...
            write_Jarvis_response("What name you want to give it sir")
            a= takeCommand()
            im.save(f"D:\YashSharma\Python\Jarvis\Screenshots\{a}.jpg")
            speak("done sir")
            write_Jarvis_response("done sir")
...
#########################################################################################################################################################
            ...
            file.write(note)
            speak("noted sir")
            write_Jarvis_response("Noted Sir")
            ...
            print(file.read())
            write_Jarvis_response(file.read())
            speak(file.read(6))
...
#########################################################################################################################################################
        ...
        elif "read website" in command or "summarize website" in command or "website summary" in command:
            speak("sir please enter url")
            write_Jarvis_response("sir please enter url")
        ...
        elif "detail website" in command or "about website" in command or "content of website" in command:
            speak("sir please enter url")
            write_Jarvis_response("sir please enter url")
            url = input("Enter the URL: ")
            if url:
...
#########################################################################################################################################################
                ...
                day = Cal_day()
                speak("Today is "+day)
                write_Jarvis_response("Today is "+day)
...
#########################################################################################################################################################
            ...
            write_Jarvis_response("On what number should I send the message sir? Please enter in the console")
            number = input("Enter the number: ")
            speak("What is the message sir?")
...
#########################################################################################################################################################
            ...
            write_Jarvis_response(f"searching for {search}")
            kit.search(search)
            ...
            command = takeCommand().lower()
            write_Jarvis_response(f"searching for {command}")
            kit.search(command)
...
#########################################################################################################################################################
        ...
        elif 'lock the window' in command:
                speak("locking the device")
                ctypes.windll.user32.LockWorkStation()
...
#########################################################################################################################################################
            ...
            speak("Playing music")
            write_Jarvis_response("Playing music")
...
#########################################################################################################################################################
            ...
            if command!="None":
                response = find_answer_local(command)
                if response:
                    print(response)
                    speak(response)
                    ...
                    try:
                        ...
                        if "explain" in command or "explanation" in command:
                            ...
                            bot_response = chat_with_gemini(f"Explain {topic} in detailed terms Reply like Tony Stark's Jarvis with respect to topic ")
                        else:
                            bot_response = chat_with_gemini(command+"***Reply like Tony Stark's Jarvis in fewer words.")
                        print(bot_response)
                        write_Jarvis_response(bot_response)
                        speak(bot_response)
                        save_to_qna(command, bot_response)
